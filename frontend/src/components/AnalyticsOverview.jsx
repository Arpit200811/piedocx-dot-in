        ...
        <div className="space-y-10 max-w-[1600px] mx-auto pb-20">
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <motion.div 
                    initial={{ opacity: 0, x: 20 }}
                    animate={{ opacity: 1, x: 0 }}
                    className="bg-slate-900 p-10 rounded-[3rem] shadow-2xl relative overflow-hidden flex flex-col h-full"
                >
                    ...
                    <div className="relative z-10 flex flex-col h-full">
                        <div className="flex items-center gap-3 mb-8">
                            <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center text-blue-400"><BarChart3 size={24} /></div>
                            <h3 className="text-xl font-black text-white uppercase italic tracking-tighter">Score Band Distribution</h3>
                        </div>
                        
                        <div className="flex-1 flex items-end gap-1 min-h-[150px] mb-8">
                            {(data.scores || []).map((s, idx) => (
                                <div key={idx} className="flex-1 group relative flex flex-col items-center gap-2">
                                    <div className="absolute -top-10 opacity-0 group-hover:opacity-100 transition-opacity bg-white text-slate-900 text-[10px] font-black px-2 py-1 rounded-lg pointer-events-none whitespace-nowrap z-20">
                                        Count: {s.count}
                                    </div>
                                    <motion.div 
                                        initial={{ height: 0 }}
                                        animate={{ height: `${(s.count / Math.max(...data.scores.map(x=>x.count), 1)) * 100}%` }}
                                        className="w-full bg-blue-500/30 group-hover:bg-blue-500 rounded-t-lg transition-colors border-x border-t border-blue-400/20"
                                    ></motion.div>
                                    <span className="text-[7px] text-slate-500 font-bold uppercase rotate-45 mt-2 origin-left">{s.range}</span>
                                </div>
                            ))}
                        </div>
