...
const TabButton = ({ active, onClick, label, Icon }) => (
  <button
    onClick={onClick}
    className={`flex items-center gap-2 px-6 py-3 rounded-xl font-black text-[9px] uppercase tracking-wider transition-all ${active
...
const AdminPanel = () => {
  ...
  const handleOldSubmit = async (e) => {
    ...
    try {
      await api.post(`/api/students`, oldStudent);
  ...
  const handleNewSubmit = async (e) => {
    ...
    setLoading(true); setSuccess(null); setError(null);
    try {
      await api.post(`/api/certificate/register`, newStudent);
  ...
  const handleBulkSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.post(endpoint, { students: jsonStudents });
      ...
      <div className="flex items-center justify-between px-2">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tighter uppercase italic flex items-center gap-2">
            <UserPlus className="text-blue-600 w-6 h-6" /> Admin Control <span className="text-blue-600">Panel</span>
          </h1>
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1 italic leading-none">System Management Console</p>
        </div>
      </div>
