...
const NavBar = () => {
  ...
  return (
    <nav
      className={`fixed top-0 w-full z-50 bg-white/70 backdrop-blur-md border-b border-white/20 shadow-sm transition-transform duration-500 ease-in-out ${
    ...
    >
      <div className="max-w-screen-2xl mx-auto px-4 md:px-8 h-14 md:h-16 lg:h-20 flex items-center justify-between font-poppins relative">
        ...
        <ul className="hidden xl:flex gap-6 font-medium text-sm lg:text-base items-center justify-center flex-1 px-4">
          {navLinks.map(({ name, path }) => (
            <li key={path} className="relative group flex-shrink-0">
              <Link
                to={path}
                className={`transition-colors duration-300 px-1 py-1 ${
                  isActive(path)
              ...
              <span
                className={`absolute left-0 -bottom-1 h-[2.5px] bg-blue-700 transition-all duration-300 group-hover:w-full ${
        ...
        <button
          ...
          className="menu-toggle xl:hidden text-gray-800 p-2"
        >
          {isOpen ? (
            <svg
      ...
      {/* Mobile Nav */}
      <div
        className={`mobile-menu xl:hidden absolute top-full left-0 w-full bg-white shadow-xl font-poppins z-50 transition-all duration-500 overflow-hidden ${
        ...
        <ul className="flex flex-col items-center gap-3 px-6 font-medium text-lg">
          {navLinks.map(({ name, path }) => (
            <li key={path} className="w-full text-center">
              <Link
                to={path}
                className={`block w-full py-3 rounded-xl transition-all duration-300 ${
                  isActive(path)
