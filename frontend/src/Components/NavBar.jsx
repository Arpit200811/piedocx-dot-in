...
import React, { useState, useEffect } from "react";
import { Link, useLocation } from "react-router-dom";
import { FaArrowUp } from "react-icons/fa"; // Page Up arrow icon
...
const NavBar = () => {
  ...
  // Hide/show nav on scroll & auto-close mobile menu
  useEffect(() => {
    const handleScroll = () => {
      const currentScrollY = window.scrollY;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={() => (isOpen ? handlePageUpClick() : setIsOpen(true))}
          className="menu-toggle lg:hidden text-gray-800 absolute right-4 z-50 flex items-center justify-center"
        >
          {isOpen ? (
            <FaArrowUp className="text-2xl text-blue-600 transition-transform duration-500 ease-in-out" />
          ) : (
            <div className="space-y-1">
              <span className="block w-6 h-[2px] bg-gray-800 rounded"></span>
              <span className="block w-6 h-[2px] bg-gray-800 rounded"></span>
              <span className="block w-6 h-[2px] bg-gray-800 rounded"></span>
            </div>
          )}
